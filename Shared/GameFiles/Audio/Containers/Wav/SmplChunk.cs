...
{
    // The standard RIFF sampler chunk. Wwise uses the first forward loop as the authored media
    // loop; the end stored in RIFF is inclusive, while the mixer works with an exclusive end.
    public sealed class SmplChunk : RiffChunk
    {
        ...
        public override void ReadData(ByteChunk chunk)
        {
            if (chunk.BytesLeft < HeaderSize)
                throw new InvalidDataException("WAV smpl chunk is shorter than its header.");
            ...
            if (chunk.BytesLeft < LoopSize)
                throw new InvalidDataException("WAV smpl chunk does not contain its declared loop.");
        ...
        public override byte[] WriteData() => throw new NotSupportedException("Writing WAV sampler metadata is not supported.");
    }
}
