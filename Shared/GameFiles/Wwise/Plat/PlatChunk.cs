...
namespace Shared.GameFormats.Wwise.Plat
{
    ...
    public class PlatChunk
    {
        ...
        public static PlatChunk ReadData(string fileName, ByteChunk chunk)
        {
            var platChunk = new PlatChunk { ChunkHeader = ChunkHeader.ReadData(chunk) };
            var stringSize = chunk.ReadUInt32();
            platChunk.CustomPlatformName = Encoding.UTF8.GetString(chunk.ReadBytes((int)stringSize)).TrimEnd('\0');
            return platChunk;
        }
    }
}
