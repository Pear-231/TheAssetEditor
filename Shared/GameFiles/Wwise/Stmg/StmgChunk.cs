...
namespace Shared.GameFormats.Wwise.Stmg
{
    ...
    {
        // The two fields whose presence depends on the bank version. Both versions this repo reads
        // are below 141, so the filter behaviour that arrives there is never present.
        private const uint DangerousVirtualVoiceLimitMinimumVersion = 127;
        ...
        public static StmgChunk ReadData(string fileName, ByteChunk chunk, uint bankGeneratorVersion)
        {
            var stmgChunk = new StmgChunk { ChunkHeader = ChunkHeader.ReadData(chunk) };
            var version = BankVersion.Normalise(bankGeneratorVersion);

            stmgChunk.VolumeThreshold = chunk.ReadSingle();
            stmgChunk.MaxVoiceLimit = chunk.ReadUShort();
            if (version >= DangerousVirtualVoiceLimitMinimumVersion)
                stmgChunk.MaxDangerousVirtualVoiceLimit = chunk.ReadUShort();
            ...
            for (var stateGroupIndex = 0; stateGroupIndex < stateGroupCount; stateGroupIndex++)
            {
                ...
                stmgChunk.StateGroups.Add(stateGroup);
            }
            ...
            for (var switchGroupIndex = 0; switchGroupIndex < switchGroupCount; switchGroupIndex++)
            {
                ...
                stmgChunk.SwitchGroups.Add(switchGroup);
            }
            ...
            for (var gameParameterIndex = 0; gameParameterIndex < gameParameterCount; gameParameterIndex++)
            {
                ...
                stmgChunk.GameParameters.Add(gameParameter);
            }

            // Acoustic textures arrive at 119, and take their current shape at 123. Both are past
            // V112, which therefore ends the chunk with its game parameters.
            if (version < AcousticTexture.MinimumVersion)
                return stmgChunk;
            ...
            for (var acousticTextureIndex = 0; acousticTextureIndex < acousticTextureCount; acousticTextureIndex++)
            {
                ...
                stmgChunk.AcousticTextures.Add(acousticTexture);
            }
            return stmgChunk;
        }
        ...
        {
            // Textures arrive at 119 in a band-filter shape and are rewritten at 123 as absorption
            // coefficients. Only the later shape is read, because neither version this repo handles
            // falls in the window between the two.
            public const uint MinimumVersion = 123;
