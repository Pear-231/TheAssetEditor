...
namespace Shared.GameFormats.Wwise.Hirc.V136.Shared
{
    public class AkDecisionTree_V136 : IAkDecisionTree
    {
        ...
        public void ReadData(ByteChunk chunk, uint treeDataSize, uint maxTreeDepth)
        {
            ...
            for (uint i = 0; i < countMax; i++)
            {
                Nodes.Add(Node_V136.ReadData(chunk, countMax, currentDepth, maxTreeDepth, i, treeDataSize));
                currentDepth++;
            }
        ...
        public class Node_V136
        {
            ...
            public static Node_V136 ReadData(ByteChunk chunk, uint countMax, uint currentDepth, uint maxDepth, uint nodeIndex, uint treeDataSize)
            {
                var node = new Node_V136();
                node.Key = chunk.ReadUInt32();

                var idChildrenPeek = chunk.PeakUint32();
                var potentialChildrenIdx = (ushort)((idChildrenPeek >> 0) & 0xFFFF);
                var potentialChildrenCount = (ushort)((idChildrenPeek >> 16) & 0xFFFF);

                var nodeSize = new Node_V136().GetSize();
                var currentOffset = nodeIndex * nodeSize;

                var isAudioNode = potentialChildrenIdx > countMax || potentialChildrenCount > countMax;
                var isOver = treeDataSize > 0 && currentOffset + potentialChildrenCount * nodeSize > treeDataSize;
                var isMax = currentDepth == maxDepth;

                if (isAudioNode || isOver || isMax)
