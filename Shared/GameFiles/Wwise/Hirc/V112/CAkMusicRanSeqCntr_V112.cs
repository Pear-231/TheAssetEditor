...
namespace Shared.GameFormats.Wwise.Hirc.V112
{
    public sealed class CAkMusicRanSeqCntr_V112 : HircItem
    {
        ...
        protected override void ReadData(ByteChunk chunk)
        {
            ...
            Playlist = PlaylistItem_V112.ReadData(chunk);
        }
        ...
        public sealed class PlaylistItem_V112
        {
            ...
            public static PlaylistItem_V112 ReadData(ByteChunk chunk)
            {
                var result = new PlaylistItem_V112
                {
                    SegmentId = chunk.ReadUInt32(), PlaylistItemId = chunk.ReadInt32(), ChildCount = chunk.ReadUInt32(),
                    SequenceType = chunk.ReadUInt32(), Loop = chunk.ReadShort(), LoopMinimum = chunk.ReadShort(),
                    LoopMaximum = chunk.ReadShort(), Weight = chunk.ReadUInt32(), AvoidRepeatCount = chunk.ReadUShort(),
                    IsUsingWeight = chunk.ReadByte(), IsShuffle = chunk.ReadByte()
                };
                for (var index = 0; index < result.ChildCount; index++) result.Children.Add(ReadData(chunk));
                return result;
            }
        }
    }
