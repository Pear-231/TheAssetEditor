...
namespace Shared.GameFormats.Wwise.Wem.V132.Encoding
{
    ...
    {
        private static readonly Lazy<SharedCodebookCache> s_sharedCache = new(CreateSharedCache, LazyThreadSafetyMode.ExecutionAndPublication);

        public static int LibraryCount => s_sharedCache.Value.ExpandedCodebooks.Length;

        public static VorbisCodebook GetCodebook(int codebookId)
        {
        ...
        public static int FindLibraryId(byte[] standardCodebookBits, int bitCount)
        {
        ...
        private static void TranscribeFromPackedEntry(BitWriter output, ReadOnlySpan<byte> codebookBytes)
        {
            ...
            var quantisedValueCount = WwiseVorbisBitstreamTranscriber.ComputeMapType1QuantValues(entries, dimensions);
        ...
        private sealed record SharedCodebookCache(Lazy<CachedCodebook>[] ExpandedCodebooks, Lazy<Dictionary<string, int>> LibraryIdByCodebookBits);
