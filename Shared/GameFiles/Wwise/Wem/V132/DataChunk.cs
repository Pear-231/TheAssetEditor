...
namespace Shared.GameFormats.Wwise.Wem.V132
{
    ...
    public class DataChunk : RiffChunk
    {
        ...
        public override void ReadData(ByteChunk chunk)
        {
            ...
            {
                var wemSeekTableRecord = new WemSeekTableRecord();
                wemSeekTableRecord.ReadData(chunk);
                SeekTable.Add(wemSeekTableRecord);
            }
            ...
            while (chunk.BytesLeft >= WemAudioPacket.LengthPrefixSize)
                {
                    var wemAudioPacket = new WemAudioPacket();
                    wemAudioPacket.ReadData(chunk);
                    AudioPackets.Add(wemAudioPacket);
                }
        }
