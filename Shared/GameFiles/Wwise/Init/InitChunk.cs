...
namespace Shared.GameFormats.Wwise.Init
{
    ...
    public class InitChunk
    {
        ...
        public static InitChunk ReadData(string fileName, ByteChunk chunk)
        {
            var initChunk = new InitChunk { ChunkHeader = ChunkHeader.ReadData(chunk) };
            initChunk.PluginCount = chunk.ReadUInt32();
            for (var pluginIndex = 0; pluginIndex < initChunk.PluginCount; pluginIndex++)
            {
                var plugin = new PluginEntry();
                plugin.ReadData(chunk);
                initChunk.Plugins.Add(plugin);
            }
            return initChunk;
        }
