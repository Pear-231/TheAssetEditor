...
namespace GameWorld.Core.Animation
{
    ...
    public class AnimationPlayer
    {
        ...
        // A finished animation is left sitting on its last frame, so playing from there would
        // have nothing left to play. Rewinding first is done before the event is raised, so
        // that anything following the player reads the position it is about to play from.
        public void Play()
        {
            var animationLengthUs = GetAnimationLengthUs();
            if (animationLengthUs > 0 && _timeSinceStart.TotalMicrosecondsAsLong >= animationLengthUs)
                _timeSinceStart = new TimeSpanExtension();
