...
namespace Test.Audio
{
    ...
    public class BankReadingTests
    {
        ...
        {
            var envs = new EnvsChunk();
            envs.ReadData("init.bnk", new ByteChunk(WarhammerEnvsChunk));
        ...
        {
            var plat = new PlatChunk();
            plat.ReadData("init.bnk", new ByteChunk(WarhammerPlatChunk));
        ...
        {
            var init = new InitChunk();
            init.ReadData("init.bnk", new ByteChunk(WarhammerInitChunk));
        ...
        public void RandomContainerPlaylistWeightsAreSignedInt32()
        {
            ...
            var v112 = new Shared.GameFormats.Wwise.Hirc.V112.CAkRanSeqCntr_V112.CAkPlayList_V112.AkPlaylistItem_V112();
            v112.ReadData(new ByteChunk(encoded));
            var v136 = new Shared.GameFormats.Wwise.Hirc.V136.CAkRanSeqCntr_V136.CAkPlayList_V136.AkPlaylistItem_V136();
            v136.ReadData(new ByteChunk(encoded));
        ...
        {
            Assert.That(
                () => WwiseVersionResolver.Resolve(135),
                Throws.TypeOf<NotSupportedException>()
                    .With.Message.EqualTo("Unsupported Wwise bank generator version: 135."));
        }
        ...
        private static byte[] SettingsChunk(uint bankVersion)
        {
            ...
            if (WwiseVersionResolver.Resolve(bankVersion).HasDangerousVirtualVoiceLimit)
            ...
            if (WwiseVersionResolver.Resolve(bankVersion).HasAcousticTextures)
