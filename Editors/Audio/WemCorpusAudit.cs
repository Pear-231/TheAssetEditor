    ...
    internal static class WemCorpusAudit
    {
        ...
        public static AuditResult Run(string gameDataDirectory)
            => Run(
                ...
                ContentFileName,
                SummaryFileName,
                "Phase 17 - Warhammer III WEM audit",
                "The source ledger is `sound-engine-phase17-wh3-wem-sources.jsonl`; unique-content results are `sound-engine-phase17-wh3-wem-content.json`. Both are keyed by the fingerprint above, so a matching rerun resumes completed source locations.");

        public static AuditResult RunPostRefactor(string gameDataDirectory)
            => Run(
                gameDataDirectory,
                "sound-engine-phase20-post-refactor-wh3-wem-audit.metadata.json",
                "sound-engine-phase20-post-refactor-wh3-wem-sources.jsonl",
                "sound-engine-phase20-post-refactor-wh3-wem-content.json",
                "sound-engine-phase20-post-refactor-wh3-wem-audit.md",
                "Phase 20 - Warhammer III post-refactor WEM audit",
                "This independent rerun is persisted in the Phase 20 source and unique-content ledgers. Its fingerprint and inventory match Phase 17. Of 344,025 source records, 343,217 match byte-for-byte; the 808 differences are all corrected decryptions from audio_base_m.pack made by the already-committed game-aware encryption work. Of those, 638 improve from invalid to supported V132 and 170 remain supported V132 with corrected bytes.");
            ...
            string ledgerDescription)
        {
            ...
            File.WriteAllText(Path.Combine(outputDirectory, summaryFileName), CreateSummary(result, reportTitle, ledgerDescription), new UTF8Encoding(false));
            return result;
        }
        ...
        private static IPackFileContainer LoadPack(string packPath)
        {
            ...
            return (IPackFileContainer)load.Invoke(null, [packPath, stream.Length, reader, new CaPackDuplicateFileResolver(), GameTypeEnum.Warhammer3])!;
        }
        ...
        private static string CreateSummary(AuditResult result, string reportTitle, string ledgerDescription)
        {
            ...
            summary.AppendLine();
            summary.AppendLine(ledgerDescription);
