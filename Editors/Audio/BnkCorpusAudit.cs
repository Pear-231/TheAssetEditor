    ...
    public static class BnkCorpusAudit
    {
        public static AuditResult Run(string game, string gameDataDirectory, string? outputDirectory = null)
        {
            ...
            for (var precedence = 0; precedence < packPaths.Length; precedence++)
            {
                ...
                try
                {
                    var pack = CorpusEnumeration.LoadPack(packPath);
            ...
            CorpusEnumeration.WriteJson(Path.Combine(outputDirectory, $"sound-engine-phase18-{game}-bnk-hirc-audit.metadata.json"), new Metadata(fingerprint, manifestFound, packPaths, finalBanks.Count, finalBanks.Count(record => record.IsEffective), result.HircCount, failures));
            CorpusEnumeration.WriteJson(Path.Combine(outputDirectory, $"sound-engine-phase18-{game}-bnk-sources.json"), finalBanks);
            CorpusEnumeration.WriteJson(Path.Combine(outputDirectory, $"sound-engine-phase18-{game}-hirc-summary.json"), result.Hircs);
            File.WriteAllText(Path.Combine(outputDirectory, $"sound-engine-phase18-{game}-bnk-hirc-initial.md"), CreateSummary(game, result), new UTF8Encoding(false));
            return result;
        }
        ...
        private static string DescribeWriterDifference(HircItem hirc, ReadOnlySpan<byte> source, ReadOnlySpan<byte> written, int firstDifference)
        {
            ...
            var node = dialogueEvent.AkDecisionTree is AkDecisionTree_V136 tree && nodeIndex < tree.Nodes.Count ? tree.Nodes[nodeIndex] : null;
        ...
        private static string GetFormatBranch(uint version) => version switch
        {
            112 => "V112",
            2147483784 => "CA-pseudo-V136",
            _ => "unresolved"
        };
        ...
        private static string CreateSummary(string game, AuditResult result)
        {
            ...
            summary.AppendLine($"# Phase 18 - {game} initial BNK/HIRC audit");
            summary.AppendLine();
            ...
            if (result.Failures.Count > 0)
            {
                ...
                summary.AppendLine("Phase 18, an enumeration failure is a hard finding, not a recorded exception, until it is reviewed.");
                summary.AppendLine();
                foreach (var failure in result.Failures)
                    summary.AppendLine($"- {failure}");
            }
            summary.AppendLine();
            summary.AppendLine("This is the initial evidence report. Per Phase 18, it must be reviewed before any parser or writer correction is made.");
            if (string.Equals(game, "wh3", StringComparison.OrdinalIgnoreCase))
            {
                summary.AppendLine();
                summary.AppendLine("The same explicit test also runs the resumable WH3 WEM audit. Its source and unique-content ledgers remain the Phase 17 files, which Phase 18 deliberately reuses as the one canonical media inventory. Attila WEMs are explicitly out of this phase's scope.");
            }
