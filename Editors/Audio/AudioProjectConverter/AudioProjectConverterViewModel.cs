...
namespace Editors.Audio.AudioProjectConverter
{
    ...
    public partial class AudioProjectConverterViewModel : ObservableObject
    {
        ...
        [RelayCommand] public void ProcessAudioProjectConversion()
        {
            ...
            if (SoundbanksInfoXmlPath != "string.empty")
        ...
        private Dictionary<uint, string> GetStatesLookupByStateID(Dictionary<string, Dictionary<uint, string>> statesLookupByStateGroupByStateID, string stateGroupLookup)
        {
            ...
            if (stateGroupLookup == "VO_Actor")
            {
                var otherStatesIDidNotMakeDatsForStupid = new List<string>
                {
                    "ovn_vo_actor_Albion_Dural_Durak",
                    "ovn_vo_actor_Albion_Mhorrinion",
                    "ovn_vo_actor_Araby_Fatandira",
                    "ovn_vo_actor_Araby_Grand_Vizier_Lord1",
                    "ovn_vo_actor_Araby_Grand_Vizier_Lord2",
                    "ovn_vo_actor_Araby_Sorcerer_Hero1",
                    "ovn_vo_actor_Araby_Sorcerer_Hero2",
                    "ovn_vo_actor_Empire_Morgan_Bernhardt",
                    "ovn_vo_actor_High_Elves_Althran_Stormrider",
                    "ovn_vo_actor_Tomb_Kings_Dread_King",
                    "str_vo_actor_Khorne_Skarr"
                };

                foreach (var stateInfo in otherStatesIDidNotMakeDatsForStupid)
                    statesLookupByStateID.TryAdd(WwiseHash.Compute(stateInfo), stateInfo);
            }
        ...
        {
            IsOkButtonEnabled = IsAudioProjectNameSet && IsOutputDirectoryPathSet && IsWemsDirectoryPathSet && IsBnksDirectoryPathSet && IsVOActorSubstringSet && (IsUsingWwiseProject && IsSoundbanksInfoXmlSet);
        }
