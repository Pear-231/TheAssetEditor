            ...
            foreach (var packPath in packPaths)
            {
                ...
                try
                {
                    ...
                    foreach (var (internalPath, file) in pack.GetAllFiles())
                    {
                        ...
                        if (game.Game.HasValue)
                            source.Parent.GameType = game.Game.Value;
                        ...
                        {
                            packResult.Failures.Add($"{internalPath}: {exception.GetType().Name}: {exception.Message} (pack-version={source.Parent.Version}, encrypted={source.IsEncrypted}, compressed={source.IsCompressed}, compression={source.CompressionFormat}, stored-size={source.Size}, uncompressed-size={source.UncompressedSize})");
                        }
                    }
                }
                catch (Exception exception)
