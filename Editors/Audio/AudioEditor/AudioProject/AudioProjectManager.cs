...
namespace Editors.Audio.AudioEditor.AudioProject
{
    public class AudioProjectManager
    {
        ...
        {
            var singleRowDataGridRow = ExtractRowFromSingleRowDataGrid(audioProjectService.DialogueEventsWithStateGroupsWithQualifiersAndStateGroupsRepository, audioRepository.StateGroupsWithStates, audioEditorViewModel.AudioProjectEditorSingleRowDataGrid, audioEditorViewModel._selectedAudioProjectTreeItem);
            ...
            else if (audioEditorViewModel._selectedAudioProjectTreeItem is DialogueEvent selectedDialogueEvent)
            {
                ...
                AddRowDataToDialogueEvent(singleRowDataGridRow, selectedDialogueEvent, audioProjectService.DialogueEventsWithStateGroupsWithQualifiersAndStateGroupsRepository);
            }
        ...
        public static void HandleUpdatingRowData(AudioEditorViewModel audioEditorViewModel, IAudioProjectService audioProjectService, IAudioRepository audioRepository)
        {
            ...
            else if (audioEditorViewModel._selectedAudioProjectTreeItem is DialogueEvent selectedDialogueEvent)
            {
                ...
                RemoveRowDataFromDialogueEvent(audioEditorViewModel.AudioProjectEditorFullDataGrid, audioEditorViewModel.SelectedDataGridRows, selectedDialogueEvent, audioProjectService);
            }
            else if (audioEditorViewModel._selectedAudioProjectTreeItem is StateGroup selectedModdedStateGroup)
            {
                ClearDataGrid(audioEditorViewModel.AudioProjectEditorSingleRowDataGrid);
        ...
        public static void RemoveRowDataFromDialogueEvent(ObservableCollection<Dictionary<string, object>> audioProjectEditorFullDataGrid, ObservableCollection<Dictionary<string, object>> dataGridRows, DialogueEvent selectedDialogueEvent, IAudioProjectService audioProjectService)
        {
            // Create a copy to prevent an error where dataGridRows is modified while being iterated over.
            var dataGridRowsCopy = dataGridRows.ToList();
            foreach (var dataGridRow in dataGridRowsCopy)
            {
                var statePath = GetStatePathFromDialogueEvent(dataGridRow, selectedDialogueEvent, audioProjectService.DialogueEventsWithStateGroupsWithQualifiersAndStateGroupsRepository);
