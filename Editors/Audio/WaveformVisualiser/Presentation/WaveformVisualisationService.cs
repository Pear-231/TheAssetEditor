...
namespace Editors.Audio.WaveformVisualiser.Presentation
{
    ...
    public sealed class WaveformVisualisationService(MediaCache mediaCache)
    {
        ...
        public Task<WaveformRenderResult> GetOrRenderWemAsync(
            ...
            => GetOrRenderAsync(
                source.CacheKey,
                targetWidth,
                () => mediaCache.GetWem(source.LoadBytes()),
                cancellationToken);
        ...
        public Task<WaveformRenderResult> GetOrRenderAudioAsync(
            ...
            int targetWidth,
            CancellationToken cancellationToken)
            => GetOrRenderAsync(sourceKey, targetWidth, () => media, cancellationToken);
            ...
            CancellationToken cancellationToken)
        {
            ...
            var renderTask = _renderInProgressByKey.GetOrAdd(
                renderKey,
                _ => Task.Run(() => Render(loadMedia(), targetWidth)));
        ...
        private static WaveformRenderResult Render(SourceMedia media, int targetWidth)
        {
            var waveFormat = WaveFormat.CreateIeeeFloatWaveFormat(media.SampleRate, media.ChannelCount);

            // NAudio's renderer reads a byte stream, so the samples are reinterpreted once here
            // rather than being kept as bytes everywhere for this one consumer's sake. It runs off
            // the UI thread, once per waveform.
            var sampleBytes = MemoryMarshal.AsBytes(media.Samples).ToArray();
            var baseWaveformBitmap = RenderWaveformImage(sampleBytes, waveFormat, CreateBaseWaveformSettings(targetWidth));
            var overlayWaveformBitmap = RenderWaveformImage(sampleBytes, waveFormat, CreateOverlayWaveformSettings(targetWidth));
            return new WaveformRenderResult(baseWaveformBitmap, overlayWaveformBitmap, media.Duration);
        }
