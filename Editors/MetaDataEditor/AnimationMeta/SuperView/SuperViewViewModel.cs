...
namespace Editors.AnimationMeta.SuperView
{
    public partial class SuperViewViewModel : EditorHostBase, ISaveableEditor
    {
        ...
        private readonly GameObjectId _previewedUnit;
        ...
        public SuperViewViewModel(
            ...
            IHircGraphService hircGraphService,
            ...
            : base(editorHostParameters)
        {
            ...
            _hircGraphService = hircGraphService;
        ...
        private void UpdateAudioSoundEventDatabaseKey()
        {
            ...
            try
            {
                ...
                if (soundEventVariable != null)
                {
                    ...
                    // Every sound the event could reach rather than the one that will be heard:
                    // a container does not choose until the cue fires.
                    var playableSourceIds = animationAudio?.PlayableSourceIds ?? [];
                    relatedValues.Add(playableSourceIds.Count == 0
                        ? "WEMs: not found"
                        : $"WEMs ({playableSourceIds.Count}): {string.Join(", ", playableSourceIds.Select(sourceId => $"{sourceId}.wem"))}");
        ...
        private void EnsureAnimationAudioCacheIsCurrent()
        {
            ...
            try
            {
                ...
                _animationAudioGame = game;
                var playableAudioCount = _animationAudioBySoundEvent.Values.Count(animationAudio => animationAudio.PlayableSourceIds.Count != 0);
            ...
            string variantMeshName)
        {
            ...
            foreach (var actionEventName in actionEventNameBySoundEventName.Values.Distinct(StringComparer.OrdinalIgnoreCase))
                switchGroupNamesByActionEventName[actionEventName] = FindSwitchGroupNames(actionEventName);
            ...
            // Told to the engine rather than carried along with every post: the switch state
            // belongs to the emitter, and the engine re-walks whatever it already holds when one
            // of them changes.
            foreach (var (switchGroupName, switchValueName) in resolvedSwitchValuesByGroupName)
                _soundEngine.SetSwitch(switchGroupName, switchValueName, _previewedUnit);
            ...
            foreach (var (actionEventName, switchGroupNames) in switchGroupNamesByActionEventName)
            {
                ...
                animationAudioByActionEventName[actionEventName] = new AnimationAudioResolution(
                    ...
                    _soundEngine.GetPlayableSourceIds(actionEventName, _previewedUnit));
            }
            ...
            return soundEvents.ToDictionary(
                ...
                soundEventName => actionEventNameBySoundEventName.TryGetValue(soundEventName, out var actionEventName)
                    ...
                    : new AnimationAudioResolution(null, [], new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase), []),
                StringComparer.OrdinalIgnoreCase);
        }

        // Which switch groups an event's containers branch on. Working out what the game would
        // have told Wwise is the game layer's job, so this stays here rather than moving into the
        // engine with the walk that consumes it.
        private string[] FindSwitchGroupNames(string actionEventName)
        {
        ...
        private void SynchroniseAudioTimeline()
        {
            ...
            var audioCues = soundTriggers
                .Select(soundTrigger => (
                    ...
                    AnimationAudio: _animationAudioBySoundEvent.GetValueOrDefault(soundTrigger.SoundEvent)))
                .Where(triggeredAudio => triggeredAudio.AnimationAudio?.ActionEvent != null
                    && triggeredAudio.AnimationAudio.PlayableSourceIds.Count != 0)
                .Select(triggeredAudio => new AnimationAudioTimeline.AudioCue(
                    ...
                    triggeredAudio.AnimationAudio!.ActionEvent!))
                .ToArray();
